...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت افاتار قـد يستغـرق الامـر 1-2 دقيقـه ▬▭ ...**",
    )
    try:
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "لـ إطفـاء الداينـو الخاص بتنصيبك بهيروكـو .. لا تستطيع اعاده التشغيل مرة اخرى عبر حسابك عليك الذهاب لحساب هيروكو واتباع الشرح التالي https://t.me/Help_Tele_Av",
...
async def _(event):
    "لـ إطفـاء البـوت"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#ايقــاف البــوت\n\n" "**- بـوت افاتار فـي وضــع الايقــاف**")
    await edit_or_reply(event, "**✾╎جــارِ إيقـاف تشغيـل بـوت افاتار الآن 📟 ...**\n\n**✾╎شغِّـلنـي يـدويًـا لاحقًــا**\n**✾╎باتبـاع الشـرح** https://t.me/Help_Tele_Av")
